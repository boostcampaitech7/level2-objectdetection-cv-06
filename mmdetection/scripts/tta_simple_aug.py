...
def apply_tta(model, img_path):
    ...
    tta_options = [
        ('original', img),  # 원본 이미지
        ('flipped', mmcv.imflip(img, direction='horizontal')),  # 좌우 반전 이미지
        ('rotated_90', mmcv.imrotate(img, angle=90)),  # 90도 회전 이미지
        ('rotated_180', mmcv.imrotate(img, angle=180))  # 180도 회전 이미지
    ]

    # 하나의 변환을 무작위로 선택
    chosen_tta_name, chosen_img = random.choice(tta_options)

    # 선택된 이미지에 대해 추론 실행
    result = inference_detector(model, chosen_img)
    ...
    if hasattr(result, 'pred_instances'):
        ...
        if det_samples is not None:
            ...
            # print(f"Applied TTA: {chosen_tta_name}")  # 적용된 변환 출력
